...
public:
	...
	My_vector(initializer_list<T>lst): sz(lst.size()),elem{alloc.allocate(sz)},space(lst.size())
	{
		//copy(lst.begin(),lst.end(),elem);
		for(int i=0;i<sz;++i) alloc.construct(&elem[i],*(lst.begin()+i));
		//debug("lst constructor");  //mettol medddig és hova
	}
	My_vector(const My_vector& arg): sz{arg.sz},elem{alloc.allocate(sz)},space{arg.sz}
	{
	...
	My_vector& operator=(const My_vector& arg)
	{
		...
		for(int i=0;i<arg.sz;++i) alloc.construct(&p[i],arg[i]);
			alloc.deallocate(elem,space); //delete helyett felszabadít....elem ptr és méret
		//delete[] elem;
		//csere=swap
		elem=p;
		sz=arg.sz;
	...
	void reserve(int newalloc)
	{
		...
		for(int i=0;i<sz;++i) alloc.destroy(&elem[i]); //delete helyett
		alloc.deallocate(elem,space);	
		//delete[] elem;
		elem= p;
		space= newalloc;
	...
	iterator erase(iterator p)
	{
		...
		for(auto pos=p+1;pos!=end();++pos)
			*(pos-1)=*pos;
		alloc.destroy(&*(end()-1)); //lássunk ilyet is :)
		--sz;
		return p;
	}
	iterator insert(iterator p, const T& val)
	{
		int index =p-begin();
		if(size()==capacity())
			reserve(size()==0?8:2*capacity()); //mindegy hogy size() vagy capacity()-t szorzunk 2 vel
...
//For testing move semantics use comand line option -fno-elide-constructors
{
	My_vector<double> t {10.2,100.3,43.2};
	return t;
}
...
try{
	...
	//test_constructors();
	/*My_vector<double> dmv{10.5,11.5,13.5};
	...
	auto p=find(smv.begin(),smv.end(),"Jill"); //pointerrel, iteratorral tér vissza find (standard függvény)
	smv.erase(p);
	smv.insert(p,"Joseph");

	for(const auto& e : smv) cout<<e<<endl;
